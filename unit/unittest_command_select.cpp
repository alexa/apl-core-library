...
TEST_F(CommandSelectTest, Basic)
{
    ...
    ASSERT_TRUE(IsEqual("Item 1", event.getValue(kEventPropertyArguments).at(0)));
...
TEST_F(CommandSelectTest, BasicSecond)
{
    ...
    auto event = root->popEvent();
    ASSERT_EQ(kEventTypeSendEvent, event.getType());
    ASSERT_TRUE(IsEqual("Item 2", event.getValue(kEventPropertyArguments).at(0)));
...
TEST_F(CommandSelectTest, DataSelection)
{
    ...
    auto event = root->popEvent();
    ASSERT_EQ(kEventTypeSendEvent, event.getType());
    ASSERT_TRUE(IsEqual("Value=3", event.getValue(kEventPropertyArguments).at(0)));
...
TEST_F(CommandSelectTest, DataSelectionByIndex)
{
    ...
    auto event = root->popEvent();
    ASSERT_EQ(kEventTypeSendEvent, event.getType());
    ASSERT_TRUE(IsEqual("Value=4", event.getValue(kEventPropertyArguments).at(0)));
...
TEST_F(CommandSelectTest, DataSelectionByIndexAndLength)
{
    ...
    auto event = root->popEvent();
    ASSERT_EQ(kEventTypeSendEvent, event.getType());
    ASSERT_TRUE(IsEqual("Value=2", event.getValue(kEventPropertyArguments).at(0)));
...
TEST_F(CommandSelectTest, DataSelectionMultipleCommands)
{
    for (const auto& m : sMultipleCommandTests) {
        ...
        component->update(kUpdatePressed, 1);
        ASSERT_TRUE(root->hasEvent()) << m.second;
        auto event = root->popEvent();
        ASSERT_EQ(kEventTypeSendEvent, event.getType()) << m.second;
        ASSERT_TRUE(IsEqual(m.second, event.getValue(kEventPropertyArguments).at(0))) << m.second;
...
TEST_F(CommandSelectTest, MultipleOtherwise)
{
    ...
    auto event = root->popEvent();
    ASSERT_EQ(kEventTypeSendEvent, event.getType());
    ASSERT_TRUE(IsEqual("alpha", event.getValue(kEventPropertyArguments).at(0)));

    ASSERT_TRUE(root->hasEvent());
    event = root->popEvent();
    ASSERT_EQ(kEventTypeSendEvent, event.getType());
    ASSERT_TRUE(IsEqual("bravo", event.getValue(kEventPropertyArguments).at(0)));
