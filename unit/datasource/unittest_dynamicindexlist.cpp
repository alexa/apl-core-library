...
static const char *DYNAMIC_DATA = R"({
  "dynamicSource": {
    "type": "dynamicIndexList",
    "listId": "vQdpOESlok",
    "startIndex": 0,
...
static const char *DYNAMIC_DATA_SEQUENCE = R"({
  "type": "APL",
  "version": "1.8",
  "mainTemplate": {
    "parameters": [
...
TEST_F(DynamicIndexListTest, VisualHashRecalculatedOnDynamicDataUpdate) {
    auto spyTextMeasure = std::make_shared<SpyTextMeasure>();
    config->measure(spyTextMeasure);

    loadDocument(DYNAMIC_DATA_SEQUENCE, DYNAMIC_DATA);
    ASSERT_TRUE(component);
    ASSERT_EQ(1, component->getChildCount());
    ASSERT_TRUE(CheckChildLaidOut(component, 0, true));
    ASSERT_EQ(1, spyTextMeasure->visualHashes.size());

    auto previousTopItem = component->findComponentById("11");
    auto previousTopItemOldVisualHash = previousTopItem->getCalculated(kPropertyVisualHash).asString();
    ASSERT_EQ(previousTopItemOldVisualHash, spyTextMeasure->visualHashes.at(0).asString());

    spyTextMeasure->visualHashes.clear();
    // Insert item on top
    ASSERT_TRUE(ds->processUpdate(createMultiInsert(1, 0, {10})));
    root->clearPending();
    ...
    auto currentTopItem = component->findComponentById("10");;
    auto currentTopItemVisualHash = currentTopItem->getCalculated(kPropertyVisualHash).asString();
    ASSERT_EQ(currentTopItemVisualHash, spyTextMeasure->visualHashes.at(0).asString());

    auto previousTopItemNewVisualHash = previousTopItem->getCalculated(kPropertyVisualHash).asString();
    // Check visual hash changed for prev top item
    ASSERT_NE(previousTopItemOldVisualHash, previousTopItemNewVisualHash);
    // Check current visual hash for current top and second item are different
    ASSERT_NE(currentTopItemVisualHash, previousTopItemNewVisualHash);
}
