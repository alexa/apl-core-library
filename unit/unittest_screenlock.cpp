...
TEST_F(ScreenLockTest, Overlapping) {
    ...
    root->updateTime(500);
    ASSERT_TRUE(root->hasEvent());
    auto send = root->popEvent();
    ASSERT_EQ(kEventTypeSendEvent, send.getType());
    ASSERT_EQ(Object("alpha"), send.getValue(kEventPropertyArguments).at(0));
    ...
    ASSERT_EQ(kEventTypeSendEvent, send.getType());
    ASSERT_EQ(Object("beta"), send.getValue(kEventPropertyArguments).at(0));
