...
TEST_F(HoverTest, Basic)
{
    ...
    root->updateCursorPosition(framePos);
    ...
    root->updateCursorPosition(textPos);
    dirty = root->getDirty();
    ...
    root->updateCursorPosition(invalidPos);
    dirty = root->getDirty();
    ASSERT_EQ(1, dirty.size());
    ASSERT_EQ(1, dirty.count(frame));
    // validate hover states
...
TEST_F(HoverTest, LOCAL_TEST)
{
    ...
    root->updateCursorPosition({50,50});
    root->clearPending();
    ...
    root->updateCursorPosition({50,150});
    root->clearPending();
    ...
    root->updateCursorPosition({300, 300});
    root->clearPending();
...
TEST_F(HoverTest, StyleUpdates_InheritedState)
{
    ...
    root->updateCursorPosition({20,20});
    root->clearPending();
    ...
    // exit all components
    root->updateCursorPosition({500,500});
    root->clearPending();
