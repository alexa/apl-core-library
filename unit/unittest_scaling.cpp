    ...
    void testRanges(const std::vector<Range>& ranges, int expectedWidth,
        ...
        for(auto dpi : dpis) {
            ...
            for (auto& range : ranges) {
                specifications.push_back({range.wmin, range.wmax, range.hmin, range.hmax, mode, isRound});
            }
            ScalingOptions options(specifications, k);
            auto result = scaling::calculate(metrics, options);
            auto m = std::get<1>(result);
            double scale = std::get<0>(result);
            ASSERT_NEAR(expectedScale, scale, 0.1);
...
TEST_F(ScalingTest, RoundAndRectSpecShapeNotMatters) {
    ...
    ScalingOptions options(specifications, k, false);
    auto result = scaling::calculate(metrics, options);
