...
GraphicContent::create(const SessionPtr& session, JsonData&& data)
{
    ...
    if (!data) {
        CONSOLE(session) << "Graphic Json could not be parsed: " << data.error();
        return nullptr;
    }

    if (!json.IsObject()) {
        CONSOLE(session) << "Unknown object type for graphic";
        return nullptr;
    }

    auto type = json.FindMember("type");
    if (type == json.MemberEnd() || !type->value.IsString()) {
        CONSOLE(session) << "Missing 'type' property in graphic";
        return nullptr;
    }

    auto typeString = type->value.GetString();
    if (::strcmp(typeString, "AVG")) {
        CONSOLE(session) << "Invalid 'type' property in graphic - must be 'AVG'";
        return nullptr;
    }

    auto version = json.FindMember("version");
    if (version == json.MemberEnd() || !version->value.IsString()) {
        CONSOLE(session) << "Missing or invalid 'version' property in graphic";
        return nullptr;
    }

    std::string v = version->value.GetString();
    if (!sGraphicVersionBimap.has(v)) {
        CONSOLE(session) << "Invalid AVG version '" << v << "'";
        return nullptr;
    }

    auto height = json.FindMember("height");
    if (height == json.MemberEnd()) {
        CONSOLE(session) << "Missing 'height' property in graphic";
        return nullptr;
    }

    auto width = json.FindMember("width");
    if (width == json.MemberEnd()) {
        CONSOLE(session) << "Missing 'width' property in graphic";
        return nullptr;
    }
