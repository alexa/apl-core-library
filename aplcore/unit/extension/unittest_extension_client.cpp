...
public:
    ...
    void createConfig(JsonData&& document) {
        ...
        configPtr->set(RootProperty::kAgentName, "Unit tests").timeManager(loop);
    ...
    void initializeContext() {
        root = std::static_pointer_cast<CoreRootContext>(RootContext::create(metrics, content, *configPtr, createCallback));
        if (root) {
...
TEST_F(ExtensionClientTest, ExtensionParseRequiredMalFormed) {
    ...
    auto client = ExtensionClient::create(configPtr, "aplext:hello:10", session);
...
TEST_F(ExtensionClientTest, MissingRootConfig) {
    createConfig(EXT_DOC);
    logBridge->clear();
    auto client = ExtensionClient::create(nullptr, "aplext:hello:10", session);
    ASSERT_FALSE(client);
    ASSERT_EQ(logBridge->getCount(), 1);
}

TEST_F(ExtensionClientTest, BadBind) {
    createConfigAndClient(EXT_DOC);
    logBridge->clear();
    client->bindContext(nullptr);
    ASSERT_EQ(logBridge->getCount(), 1);
}
...
TEST_F(ExtensionClientTest, ExtensionParseRequired) {
    ...
    ASSERT_EQ("aplext:hello:10", client->getUri());

    // Extension client does not directly modify RootConfig
    auto ext = configPtr->getSupportedExtensions();
    ASSERT_EQ(0, ext.size());
}

static const char* BAD_METHOD = R"({
  "method": "Oops",
  "version": "1.0",
  "token": "TOKEN-12",
  "extension": "aplext:hello:10"
...
TEST_F(ExtensionClientTest, ExtensionBadMethod) {
    createConfigAndClient(EXT_DOC);
    ASSERT_FALSE(client->processMessage(nullptr, BAD_METHOD));
    ASSERT_TRUE(ConsoleMessage());
}

static const char* BAD_REGISTRATION_MESSAGE = R"({
  "method": "RegisterSuccess",
  "version": "1.0",
  "extension": "aplext:hello:10"
})";

TEST_F(ExtensionClientTest, ExtensionBadRegistrationResponse) {
    createConfigAndClient(EXT_DOC);
    ASSERT_FALSE(client->processMessage(nullptr, BAD_REGISTRATION_MESSAGE));
    ASSERT_TRUE(ConsoleMessage());
}
...
TEST_F(ExtensionClientTest, ExtensionParseCommands) {
    ...
    ASSERT_TRUE(client->processMessage(nullptr, doc));
    ASSERT_FALSE(ConsoleMessage());
    const auto &commands = client->extensionSchema().commandDefinitions;
...
TEST_F(ExtensionClientTest, ExtensionParseEventHandlers) {
    ...
    const auto &handlers = client->extensionSchema().eventHandlers;
...
TEST_F(ExtensionClientTest, ExtensionParseComponent) {
    ...
    const auto &components = client->extensionSchema().componentDefinitions;
...
TEST_F(ExtensionClientTest, Registered) {
    ...
    const auto &env = client->extensionSchema().environment;
    ...
    const auto &events = client->extensionSchema().eventHandlers;
    ASSERT_EQ(6, events.size());
    auto event = events.at(0);
    ASSERT_EQ("onEntityAdded", event.getName());

    const auto &liveData = client->extensionSchema().liveData;
...
TEST_F(ExtensionClientTest, SimpleEvent) {
    createConfigAndClient(EXT_DOC);
...
TEST_F(ExtensionClientTest, ManyClients) {
    ...
    ASSERT_FALSE(ConsoleMessage());
    ASSERT_TRUE(client1->registered());

    client2->processMessage(nullptr, EXT_REGISTER_GREETINGS);
    ASSERT_FALSE(ConsoleMessage());
    ASSERT_TRUE(client2->registered());
}
...
TEST_F(ExtensionClientTest, CommandBadEventType) {
    createConfig(EXT_DOC);
    client = ExtensionClient::create(config, "aplext:hello:10", session);

    initializeContext();

    EventBag bag;
    bag.emplace(kEventPropertyName, "arbitraryName");
    Event event(apl::kEventTypeSendEvent, std::move(bag));
    ASSERT_TRUE(client->processCommand(doc.GetAllocator(), event).GetType() == rapidjson::kNullType);
    ASSERT_TRUE(ConsoleMessage());
}

TEST_F(ExtensionClientTest, CommandBadExtension) {
    createConfig(EXT_DOC);
    client = ExtensionClient::create(config, "aplext:hello:10", session);
    ...
    EventBag bag;
    Event event(apl::kEventTypeExtension, std::move(bag));
    ASSERT_TRUE(client->processCommand(doc.GetAllocator(), event).GetType() == rapidjson::kNullType);
    ASSERT_TRUE(ConsoleMessage());
}

TEST_F(ExtensionClientTest, CommandBadCommandName) {
    createConfig(EXT_DOC);
    client = ExtensionClient::create(config, "aplext:hello:10", session);
    ...
    EventBag bag;
    bag.emplace(kEventPropertyExtensionURI, "aplext:hello:10");
    Event event(apl::kEventTypeExtension, std::move(bag));
    ASSERT_TRUE(client->processCommand(doc.GetAllocator(), event).GetType() == rapidjson::kNullType);
    ASSERT_TRUE(ConsoleMessage());
}
...
TEST_F(ExtensionClientTest, CommandBadResourceId) {
    ...
    bag.emplace(kEventPropertyName, "arbitraryName");
    bag.emplace(kEventPropertyExtensionURI, "aplext:hello:10");
    bag.emplace(kEventPropertyExtensionResourceId, 7);
    Event event(apl::kEventTypeExtension, std::move(bag));
    ASSERT_TRUE(client->processCommand(doc.GetAllocator(), event).GetType() == rapidjson::kNullType);
    ASSERT_TRUE(ConsoleMessage());
}
...
static const char* EXT_INIT_DATA_DOC = R"({
  "type": "APL",
  "version": "2023.2",
  "extension": {
    "uri": "aplext:hello:10",
  ...
  "mainTemplate": {
    "item": {
      ...
      "width": 500,
      "height": 100,
      "text": "${EntityMap.label ? EntityMap.label : 'nope'}"
    }
  }
})";

static const char* EXT_REGISTER_WITHOUT_DATA_SUCCESS = R"({
  "method": "RegisterSuccess",
  "version": "1.0",
  "token": "TOKEN",
  "schema": {
...
static const char* INSERT_INIT_DATA = R"({
  "version": "1.0",
  "method": "LiveDataUpdate",
  "name": "EntityMap",
  "target": "aplext:hello:10",
...
TEST_F(ExtensionClientTest, InitialDataFollowup) {
    createConfigAndClient(EXT_INIT_DATA_DOC);

    ASSERT_TRUE(client->processMessage(nullptr, EXT_REGISTER_WITHOUT_DATA_SUCCESS));
    ASSERT_FALSE(ConsoleMessage());

    initializeContext();
    ASSERT_TRUE(client->processMessage(nullptr, INSERT_INIT_DATA));

    client->bindContext(root);

    ASSERT_EQ("nope", component->getCalculated(kPropertyText).asString());
}

static const char* EXT_REGISTER_WITH_DATA_SUCCESS = R"({
  "method": "RegisterSuccess",
  "version": "1.0",
  "token": "TOKEN",
  "schema": {
...
TEST_F(ExtensionClientTest, InitialDataInRegistration) {
    createConfigAndClient(EXT_INIT_DATA_DOC);

    ASSERT_TRUE(client->processMessage(nullptr, EXT_REGISTER_WITH_DATA_SUCCESS));
    ASSERT_FALSE(ConsoleMessage());
    ...
    ASSERT_EQ("Hello there", component->getCalculated(kPropertyText).asString());
}

static const char* EXT_INIT_ARRAY_DOC = R"({
  "type": "APL",
  "version": "2023.2",
  "extension": {
    "uri": "aplext:hello:10",
...
static const char* EXT_REGISTER_WITH_ARRAY_SUCCESS = R"({
  "method": "RegisterSuccess",
  "version": "1.0",
  "token": "TOKEN",
  "schema": {
...
TEST_F(ExtensionClientTest, InitialArrayInRegistration) {
    createConfigAndClient(EXT_INIT_ARRAY_DOC);

    ASSERT_TRUE(client->processMessage(nullptr, EXT_REGISTER_WITH_ARRAY_SUCCESS));
    ASSERT_FALSE(ConsoleMessage());
    ...
    ASSERT_EQ("Hello there", component->getChildAt(0)->getCalculated(kPropertyText).asString());
    ASSERT_EQ("Bye now", component->getChildAt(1)->getCalculated(kPropertyText).asString());
}

static const char* EXT_REGISTER_SUCCESS_WITH_BAD_DATA = R"({
  "method": "RegisterSuccess",
  "version": "1.0",
  "token": "TOKEN",
  "schema": {
...
TEST_F(ExtensionClientTest, InitialBadStartingData) {
    createConfigAndClient(EXT_INIT_ARRAY_DOC);

    ASSERT_TRUE(client->processMessage(nullptr, EXT_REGISTER_SUCCESS_WITH_BAD_DATA));

    initializeContext();

    client->bindContext(root);

    ASSERT_TRUE(session->checkAndClear("Initial data for LiveData=EntityArray is of invalid type. Should be an Array.",
                                       "Initial data for LiveData=EntityMap is of invalid type. Should be a Map."));
}
...
TEST_F(ExtensionClientTest, TypeWithoutProperties) {
    createConfigAndClient(EXT_DOC);
...
TEST_F(ExtensionClientTest, ExtensionComponentCommandAndEvent) {
    ...
    auto touchwrapper = root->findComponentById("AlexaButton");
    ...
    auto extensionComponent = root->findComponentById("DrawArea");
...
TEST_F(ExtensionClientTest, ExtensionComponentKPropOutProperty) {
    ...
    auto alexaButton = root->findComponentById("AlexaButton");
    auto extensionComponent = CoreComponent::cast(root->findComponentById("DrawArea"));
    ASSERT_EQ(extensionComponent->getType(), kComponentTypeExtension);
