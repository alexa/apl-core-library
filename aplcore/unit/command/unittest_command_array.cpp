...
public:
    ...
    static CommandPtr createFull(const ContextPtr& context,
...
public:
    ArrayCommandTest()
        : ActionWrapper()
    {
        CommandFactory::instance().set("Test", TestCommand::createFull);
        TestCommand::sSum = 0;
...
TEST_F(ArrayCommandTest, MultipleCommand)
{
    ...
    auto command = ArrayCommand::create(context, Object(json.get()), nullptr, Properties());
...
{
    auto json = JsonData(BASIC);
    auto command = ArrayCommand::create(context, Object(json.get()), nullptr, Properties(), "", true);
    auto action = command->execute(loop, false);
