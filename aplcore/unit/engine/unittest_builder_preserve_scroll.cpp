...
static const char *SCROLL_VIEW_INSIDE_PAGER = R"(
        {
          "type": "APL",
          "version": "2022.2",
          "onConfigChange": {
...
TEST_F(BuilderPreserveScrollTest, PreserveScrollPercentFirstPage)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER, R"({"preserve": "scrollPercent"})");

    ASSERT_TRUE(component);
    auto sv = root->findComponentById("sv-red");
    ASSERT_TRUE(sv);

    ASSERT_TRUE(IsEqual(Dimension(0), sv->getCalculated(kPropertyScrollPosition)));
    sv->update(kUpdateScrollPosition, 100); // 50% of viewport height
    ASSERT_TRUE(IsEqual(Dimension(100), sv->getCalculated(kPropertyScrollPosition)));

    configChangeReinflate(ConfigurationChange(200, 400)); // portrait

    sv = root->findComponentById("sv-red");

    ASSERT_TRUE(sv);
    ASSERT_TRUE(IsEqual(Dimension(200), sv->getCalculated(kPropertyScrollPosition))); // scroll position is still half of the window
}

TEST_F(BuilderPreserveScrollTest, PreserveScrollPercentSecondPage)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER, R"({"preserve": "scrollPercent"})");
    ASSERT_TRUE(component);
    ...
    auto sv = root->findComponentById("sv-blue");
    ASSERT_TRUE(sv);
    ...
    sv = root->findComponentById("sv-blue");

    ASSERT_TRUE(sv);
    ASSERT_TRUE(IsEqual(Dimension(200), sv->getCalculated(kPropertyScrollPosition))); // scroll position is still half of the window
}

TEST_F(BuilderPreserveScrollTest, PreserveScrollPercentHiddenPage)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER, R"({"preserve": "scrollPercent"})");
    ASSERT_TRUE(component);
    ...
    auto red = root->findComponentById("sv-red");
    ASSERT_TRUE(red);

    // Scroll the first page
    ASSERT_TRUE(IsEqual(Dimension(0), red->getCalculated(kPropertyScrollPosition)));
    red->update(kUpdateScrollPosition, 100); // 50% of viewport height
    ASSERT_TRUE(IsEqual(Dimension(100), red->getCalculated(kPropertyScrollPosition)));

    // Switch to page 2
    executeCommand("SetPage", {{"componentId", "pager"}, {"value", 1}}, false); // switch to page 2
    advanceTime(1000);

    // Scroll the second page
    auto blue = root->findComponentById("sv-blue");
    ASSERT_TRUE(blue);

    ASSERT_TRUE(IsEqual(Dimension(0), blue->getCalculated(kPropertyScrollPosition)));
    blue->update(kUpdateScrollPosition, 200); // 100% of viewport height
    ASSERT_TRUE(IsEqual(Dimension(200), blue->getCalculated(kPropertyScrollPosition)));

    // Reinflate
    configChangeReinflate(ConfigurationChange(200, 400)); // portrait

    // Check the second page
    blue = root->findComponentById("sv-blue");
    ASSERT_TRUE(blue);
    ASSERT_TRUE(IsEqual(Dimension(400), blue->getCalculated(kPropertyScrollPosition))); // scroll position is still 100% of the window

    // Switch to page 1
    executeCommand("SetPage", {{"componentId", "pager"}, {"value", 0}}, false);
    advanceTime(1000);

    // Check the first page
    red = root->findComponentById("sv-red");
    ASSERT_TRUE(red);
    ASSERT_TRUE(IsEqual(Dimension(200), red->getCalculated(kPropertyScrollPosition))); // scroll position is still 50% of the window
}

TEST_F(BuilderPreserveScrollTest, PreserveScrollOffsetFirstPage)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER, R"({"preserve": "scrollOffset"})");
    advanceTime(500);
    ...
    ASSERT_TRUE(IsEqual(Dimension(0), sv->getCalculated(kPropertyScrollPosition)));
    sv->update(kUpdateScrollPosition, 100);
    ASSERT_TRUE(IsEqual(Dimension(100), sv->getCalculated(kPropertyScrollPosition)));
    advanceTime(1000);
    ...
    ASSERT_TRUE(sv);
    ASSERT_TRUE(IsEqual(Dimension(100), sv->getCalculated(kPropertyScrollPosition))); // scroll position is still 100
}

TEST_F(BuilderPreserveScrollTest, PreserveScrollOffsetSecondPage)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER, R"({"preserve": "scrollOffset"})");
    ASSERT_TRUE(component);
...
TEST_F(BuilderPreserveScrollTest, PreserveScrollOffsetHiddenPage)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER, R"({"preserve": "scrollOffset"})");
    ASSERT_TRUE(component);
    ...
    // Scroll the first page
    ASSERT_TRUE(IsEqual(Dimension(0), red->getCalculated(kPropertyScrollPosition)));
    red->update(kUpdateScrollPosition, 100);
    ASSERT_TRUE(IsEqual(Dimension(100), red->getCalculated(kPropertyScrollPosition)));
    ...
    ASSERT_TRUE(IsEqual(Dimension(0), blue->getCalculated(kPropertyScrollPosition)));
    blue->update(kUpdateScrollPosition, 150);
    ASSERT_TRUE(IsEqual(Dimension(150), blue->getCalculated(kPropertyScrollPosition)));
    ...
    blue = root->findComponentById("sv-blue");
    ASSERT_TRUE(blue);
    ASSERT_TRUE(IsEqual(Dimension(150), blue->getCalculated(kPropertyScrollPosition))); // scroll position is still 150
    ...
    red = root->findComponentById("sv-red");
    ASSERT_TRUE(red);
    ASSERT_TRUE(IsEqual(Dimension(100), red->getCalculated(kPropertyScrollPosition))); // scroll position is still 100
}

static const char *SCROLL_VIEW_INSIDE_PAGER_INDIRECT = R"(
        {
          "type": "APL",
          "version": "2022.2",
          "onConfigChange": {
...
TEST_F(BuilderPreserveScrollTest, PreserveScrollPercentFirstPageIndirect)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER_INDIRECT, R"({"preserve": "scrollPercent"})");
...
TEST_F(BuilderPreserveScrollTest, PreserveScrollOffsetFirstPageIndirect)
{
    metrics.size(400, 200); // landscape
    loadDocument(SCROLL_VIEW_INSIDE_PAGER_INDIRECT, R"({"preserve": "scrollOffset"})");
