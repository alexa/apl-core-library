...
TEST_F(GesturesTest, SwipeAwayUnfinished)
{
    ...
    auto tw = TouchWrapperComponent::cast(root->findComponentById("tw"));
...
TEST_F(GesturesTest, DoublePressDisabledAVG)
{
    ...
    auto myGraphic = CoreComponent::cast(root->findComponentById("MyGraphic"));
...
TEST_F(GesturesTest, SwipeAwayRTL) {
    ...
    auto f1 = CoreComponent::cast(root->findComponentById("forwardSwipe"));
    auto f2 = CoreComponent::cast(root->findComponentById("backwardSwipe"));
...
static const char * SWIPE_CHILD_THEN_SWIPE_PARENT = R"apl({
  "type": "APL",
  "version": "1.9",
  "mainTemplate": {
      "items": [{
...
TEST_F(GesturesTest, SwipeChildThenSwipParent)
{
    loadDocument(SWIPE_CHILD_THEN_SWIPE_PARENT);
    ASSERT_TRUE(component);

    auto yellowFrame = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_TRUE(IsEqual(Color(Color::YELLOW), yellowFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,400,200}, yellowFrame->getCalculated(kPropertyBounds)));

    // Execute a swipe
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(10,20))));
    advanceTime(10);
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(10,20))));
    advanceTime(250);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(300,20))));
    advanceTime(10);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(300,20))));
    advanceTime(1000);

    // Child's yellow frame is gone
    auto yellowFrameGone = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_FALSE(IsEqual(Color(Color::YELLOW), yellowFrameGone->getCalculated(apl::kPropertyBackgroundColor)));

    auto blueFrame = component->getChildAt(0);
    ASSERT_TRUE(IsEqual(Color(Color::BLUE), blueFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,200}, blueFrame->getCalculated(kPropertyBounds)));

    // Execute a swipe at the same place
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(10,20))));
    advanceTime(10);
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(10,20))));
    advanceTime(250);
    ...
    // Parent's blue frame is gone
    auto redFrame = component->getChildAt(0);
    ASSERT_NE(redFrame, blueFrame);
    ASSERT_TRUE(IsEqual(Color(Color::RED), redFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,200}, redFrame->getCalculated(kPropertyBounds)));
}

static const char * PARENT_CHILD_BOTH_HAVE_DOUBLEPRESS = R"apl({
    "type": "APL",
    "version": "1.9",
    "mainTemplate": {
        "items": [{
            ...
            "items": [{
                ...
                "gestures": [{
                    ...
                    "onDoublePress": [{
                        "type": "SetValue",
                        "componentId": "parentText",
                        "property": "text",
                        "value": "Parent detect DoublePress."
                    }]
                }],
                "items": [{
                    "type": "Frame",
                        ...
                        "items": [{
                            ...
                            "items": [{
                                ...
                                "gestures": [{
                                    ...
                                    "onDoublePress": [{
                                        "type": "SetValue",
                                        "componentId": "childText",
                                        "property": "text",
                                        "value": "Kid detect DoublePress."
                                    }]
                                }],
                                "items": [{
                                    "type": "Frame",
...
TEST_F(GesturesTest, OnlyChildGetDoublePress)
{
    loadDocument(PARENT_CHILD_BOTH_HAVE_DOUBLEPRESS);
    ASSERT_TRUE(component);

    auto parentText = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_EQ(kComponentTypeText, parentText->getType());
    ASSERT_EQ("Lorem ipsum dolor", parentText->getCalculated(kPropertyText).asString());

    auto childText = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(1);
    ASSERT_EQ(kComponentTypeText, childText->getType());
    ASSERT_EQ("Lorem ipsum dolor", childText->getCalculated(kPropertyText).asString());

    // Trigger a DoublePress
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(100,300))));
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(100,300))));
    advanceTime(400);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(100,300))));
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(100,300))));

    // Verify only the kid get the DoublePress
    parentText = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_EQ(kComponentTypeText, parentText->getType());
    ASSERT_EQ("Lorem ipsum dolor", parentText->getCalculated(kPropertyText).asString());

    childText = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(1);
    ASSERT_EQ(kComponentTypeText, childText->getType());
    ASSERT_EQ("Kid detect DoublePress.", childText->getCalculated(kPropertyText).asString());

}

static const char * PARENT_CHILD_BOTH_HAVE_ONDOWN = R"apl({
    "type": "APL",
    "version": "1.9",
    "mainTemplate": {
        "items": [{
...
TEST_F(GesturesTest, OnlyChildGetOnDown)
{
    loadDocument(PARENT_CHILD_BOTH_HAVE_ONDOWN);
    ASSERT_TRUE(component);
    ...
    // Trigger an onDown on overlapping area
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(200,300))));
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(200,300))));
    advanceTime(10);

    // Only kid received the onDown
    parentText = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_EQ(kComponentTypeText, parentText->getType());
    ASSERT_EQ("Lorem ipsum dolor", parentText->getCalculated(kPropertyText).asString());
    ...
    childText = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(1);
    ASSERT_EQ(kComponentTypeText, childText->getType());
    ASSERT_EQ("Kid detect onDown.", childText->getCalculated(kPropertyText).asString());

    // Trigger an onDown on parent where child doesn't have any overlap.
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(600,300))));
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(600,300))));
    advanceTime(10);

    // Parent received the onDown
    parentText = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_EQ(kComponentTypeText, parentText->getType());
    ASSERT_EQ("Parent detect onDown.", parentText->getCalculated(kPropertyText).asString());
}

static const char * DOUBLEPRESS_CHILD_THEN_SWIPE_PARENT = R"apl({
    "type": "APL",
    "version": "1.9",
    "mainTemplate": {
        "items": [{
...
TEST_F(GesturesTest, DoublePressChildThenSwipParent)
{
    loadDocument(DOUBLEPRESS_CHILD_THEN_SWIPE_PARENT);
    ASSERT_TRUE(component);

    auto text = component->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_EQ(kComponentTypeText, text->getType());
    ASSERT_EQ("Lorem ipsum dolor", text->getCalculated(kPropertyText).asString());

    // Trigger a DoublePress
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(15,100))));
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(15,100))));
    advanceTime(400);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(15,100))));
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(15,100))));

    // Verify the DoublePress
    ASSERT_TRUE(CheckEvent("onDoublePress", 15));
    ASSERT_EQ("Detect DoublePress.", text->getCalculated(kPropertyText).asString());
    ...
    // Execute a swipe
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(10,100))));
    advanceTime(10);
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(10,100))));
    advanceTime(250);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(300,100))));
    advanceTime(10);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(300,100))));
    advanceTime(1000);
...
static const char * ONDOWN_CHILD_THEN_SWIPE_PARENT = R"apl({
    "type": "APL",
    "version": "1.9",
    "mainTemplate": {
        "items": [{
...
TEST_F(GesturesTest, OnDownChildThenSwipeParent)
{
    loadDocument(ONDOWN_CHILD_THEN_SWIPE_PARENT);
    ASSERT_TRUE(component);
    ...
    // Verify the kid has got the onDown
    ASSERT_EQ("Kid detect onDown.", text->getCalculated(kPropertyText).asString());
...
static const char * THREE_LAYERS_SWIPE_WITH_DIFFERENT_DIRECTIONS = R"apl({
    "type": "APL",
    "version": "1.9",
    "mainTemplate": {
        "items": [{
...
{
    loadDocument(THREE_LAYERS_SWIPE_WITH_DIFFERENT_DIRECTIONS);
    ASSERT_TRUE(component);

    auto greenFrame = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_TRUE(IsEqual(Color(Color::GREEN), greenFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,400}, greenFrame->getCalculated(kPropertyBounds)));

    auto yellowFrame = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_TRUE(IsEqual(Color(Color::YELLOW), yellowFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,400}, yellowFrame->getCalculated(kPropertyBounds)));

    auto blueFrame = component->getChildAt(0);
    ASSERT_TRUE(IsEqual(Color(Color::BLUE), blueFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,400}, blueFrame->getCalculated(kPropertyBounds)));

    // Execute a swipe left
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(300,20))));
    advanceTime(10);
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(300,20))));
    advanceTime(250);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(10,20))));
    advanceTime(10);
    ASSERT_TRUE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerUp, Point(10,20))));
    ...
    // blue frame's child are gone
    auto blueFrameChildCount = component->getChildAt(0)->getChildCount();
    ASSERT_TRUE(IsEqual(0, blueFrameChildCount));

    // blue frame is gone
    auto redFrame = component->getChildAt(0);
    ASSERT_NE(redFrame, blueFrame);
    ASSERT_TRUE(IsEqual(Color(Color::RED), redFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,400}, redFrame->getCalculated(kPropertyBounds)));
}

TEST_F(GesturesTest, MidLayerGetSwipeRight)
{
    loadDocument(THREE_LAYERS_SWIPE_WITH_DIFFERENT_DIRECTIONS);
    ASSERT_TRUE(component);
    ...
    // yellow frame's child is gone
    auto yellowFrameChildCount = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildCount();
    ASSERT_TRUE(IsEqual(0, yellowFrameChildCount));

    // yellow frame is gone
    auto yellowFrameGone = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_FALSE(IsEqual(Color(Color::YELLOW), yellowFrameGone->getCalculated(apl::kPropertyBackgroundColor)));

    // blue frame is still here
    auto stillBlueFrame = component->getChildAt(0);
    ASSERT_EQ(stillBlueFrame, blueFrame);
    ASSERT_TRUE(IsEqual(Color(Color::BLUE), stillBlueFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,400}, stillBlueFrame->getCalculated(kPropertyBounds)));
}

TEST_F(GesturesTest, BottonLayerGetSwipeUp)
{
    loadDocument(THREE_LAYERS_SWIPE_WITH_DIFFERENT_DIRECTIONS);
    ASSERT_TRUE(component);
    ...
    // Execute a swipe up
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerDown, Point(50,300))));
    advanceTime(10);
    ASSERT_FALSE(root->handlePointerEvent(PointerEvent(PointerEventType::kPointerMove, Point(50,300))));
    advanceTime(250);
    ...
    // green Frame is gone
    auto greenFrameGone = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_FALSE(IsEqual(Color(Color::GREEN), greenFrameGone->getCalculated(apl::kPropertyBackgroundColor)));

    // yellow Frame is still here
    auto stillYellowFrame = component->getChildAt(0)->getChildAt(0)->getChildAt(0)->getChildAt(0);
    ASSERT_EQ(stillYellowFrame, yellowFrame);
    ASSERT_TRUE(IsEqual(Color(Color::YELLOW), stillYellowFrame->getCalculated(apl::kPropertyBackgroundColor)));
    ASSERT_TRUE(IsEqual(Rect{0,0,800,400}, stillYellowFrame->getCalculated(kPropertyBounds)));
