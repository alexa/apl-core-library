...
static const char *LIVE_DATA_PAGER = R"({
  "type": "APL",
  "version": "1.6",
  "mainTemplate": {
    "items": {
...
TEST_F(NativeGesturesPagerTest, LiveDataPagerFullReplacement)
{
    auto liveArray = LiveArray::create({"red", "green", "yellow", "blue", "purple", "gray", "red", "green", "yellow", "blue", "purple", "gray"});
    config->liveData("colors", liveArray);
    loadDocument(LIVE_DATA_PAGER);

    // We are positioned at origin and first of 12 pages is shown
    ASSERT_EQ(Point(), component->scrollPosition());
    ASSERT_EQ(1, component->getDisplayedChildCount());
    ASSERT_EQ("red0", component->getDisplayedChildAt(0)->getId());
    ...
    // Start paging a bit
    auto ptr = executeCommand("AutoPage", {{"componentId", "pagers"}, {"count", 1}, {"duration", 100}}, false);
    advanceTime(200);

    // We are partway between the first and second page
    ASSERT_FALSE(ptr->isResolved());
    ASSERT_EQ(2, component->getDisplayedChildCount());
    ASSERT_EQ("red0", component->getDisplayedChildAt(0)->getId());
    ASSERT_EQ("green1", component->getDisplayedChildAt(1)->getId());
    ASSERT_NE(Object(Transform2D()), component->getDisplayedChildAt(0)->getCalculated(kPropertyTransform));
    ASSERT_NE(Object(Transform2D()), component->getDisplayedChildAt(1)->getCalculated(kPropertyTransform));

    // We simulate the VH holding strong references to these pages (although in practice there are other strong references currently)
    auto red = component->getDisplayedChildAt(0);
    auto green = component->getDisplayedChildAt(1);

    // Fully replace the live data powering the pager
    liveArray->remove(0, liveArray->size());
    liveArray->insert(0, "black");
    liveArray->insert(0, "white");
    ASSERT_EQ(2, liveArray->size());

    // A little more time passes, which is not enough to complete the page transition
    advanceTime(200);

    // There is no event and for a moment, no children are displayed
    // TODO: Should the page transition be fast-forwarded to the next existing page?
    ASSERT_FALSE(ptr->isResolved());
    ASSERT_FALSE(root->hasEvent());
    ASSERT_EQ(0, component->getDisplayedChildCount());

    // Eventually, the pagination completes
    advanceTime(1000);
...
TEST_F(NativeGesturesPagerTest, LiveDataTargetOrCurrentRemove)
{
    auto liveArray = LiveArray::create({"red", "green", "yellow", "blue", "purple", "gray", "red", "green", "yellow", "blue", "purple", "gray"});
    config->liveData("colors", liveArray);
    loadDocument(LIVE_DATA_PAGER);
    ...
    // Remove the target page (green)
    liveArray->remove(1);
    ASSERT_EQ(11, liveArray->size());

    // A little more time passes, which is not enough to complete the page transition
    advanceTime(200);

    // There is no event and the current page is still visible
    ASSERT_FALSE(ptr->isResolved());
    ASSERT_FALSE(root->hasEvent());
    ASSERT_EQ(1, component->getDisplayedChildCount());
    ASSERT_EQ("red0", component->getDisplayedChildAt(0)->getId());
    ...
    // Eventually, the transition completes
    advanceTime(1000);
    ...
    // Try again
    ptr = executeCommand("AutoPage", {{"componentId", "pagers"}, {"count", 1}, {"duration", 100}}, false);
    advanceTime(200);
    ...
    ASSERT_EQ(2, component->getDisplayedChildCount());
    ASSERT_EQ("red0", component->getDisplayedChildAt(0)->getId());
    // TODO: This should be "yellow1" because the second item is gone
    ASSERT_EQ("yellow2", component->getDisplayedChildAt(1)->getId());
    ASSERT_NE(Object(Transform2D()), component->getDisplayedChildAt(0)->getCalculated(kPropertyTransform));
    ASSERT_NE(Object(Transform2D()), component->getDisplayedChildAt(1)->getCalculated(kPropertyTransform));

    // This time, remove the current page
    liveArray->remove(0);
    ASSERT_EQ(10, liveArray->size());

    // A little more time passes, which is not enough to complete the page transition
    advanceTime(200);

    // There is no event and the target page is still visible
    ASSERT_FALSE(ptr->isResolved());
    ASSERT_FALSE(root->hasEvent());
    ASSERT_EQ(1, component->getDisplayedChildCount());
    // TODO: Again this should be "yellow1"
    ASSERT_EQ("yellow2", component->getDisplayedChildAt(0)->getId());
    ASSERT_NE(Object(Transform2D()), component->getDisplayedChildAt(0)->getCalculated(kPropertyTransform));
