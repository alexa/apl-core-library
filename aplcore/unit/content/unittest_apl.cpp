...
TEST(APLTest, Basic)
{
    ...
    auto content = Content::create(MAIN, session);
    ASSERT_TRUE(content);
    ...
    ASSERT_EQ(Object(Color(session, "#ff1020")), text->getCalculated(kPropertyColor));
    ...
    ASSERT_EQ(Object(Color(session, "blue")), text->getCalculated(kPropertyColor));
    root->clearDirty();
    ...
    // Check findByURI API
    ASSERT_EQ(
        static_cast<Component*>(root->findByUniqueId(root->topComponent()->getUniqueId())),
        root->topComponent().get());
}
