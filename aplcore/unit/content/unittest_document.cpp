...
TEST(DocumentTest, ExternalCommandTest)
{
    ...
    ASSERT_TRUE(root);
    ASSERT_STREQ("Is Not Set", root->topComponent()->getCalculated(kPropertyText).asString().c_str());

    auto cmd = JsonData(EXTERNAL_COMMAND_TEST_COMMAND);
    ASSERT_TRUE(cmd);

    root->topDocument()->executeCommands(cmd.get(), false);
    ASSERT_STREQ("Is Set", root->topComponent()->getCalculated(kPropertyText).asString().c_str());
}
...
TEST(DocumentTest, LogId)
{
    ...
    auto content = Content::create(NO_DIAGNOSTIC_TAG, session);
    ASSERT_TRUE(content->isReady());
    ...
    ASSERT_EQ(10, session->getLogId().size());
    LOG(LogLevel::kInfo).session(session) << "TEST";
    ASSERT_EQ(session->getLogId() + ":unittest_document.cpp:TestBody : TEST", logBridge->mLog);
...
TEST(DocumentTest, ShortLogId)
{
    ...
    auto session = makeDefaultSession();
    auto content = Content::create(LOG_ID_WITH_PREFIX, session);
    ASSERT_TRUE(content->isReady());
    ...
    logBridge->reset();
    LOG(LogLevel::kInfo).session(session) << "TEST";
    ASSERT_EQ(17, session->getLogId().size());
    ASSERT_EQ(session->getLogId() + ":unittest_document.cpp:TestBody : TEST", logBridge->mLog);
...
TEST(DocumentTest, TwoDocuments)
{
    ...
    auto session1 = makeDefaultSession();
    ...
    LOG(LogLevel::kInfo).session(session1) << "TEST";
    ASSERT_EQ(17, session1->getLogId().size());
    ASSERT_EQ(session1->getLogId() + ":unittest_document.cpp:TestBody : TEST", logBridge->mLog);

    LOG(LogLevel::kInfo).session(session2) << "TEST";
    ASSERT_EQ(17, session2->getLogId().size());
    ASSERT_EQ(session2->getLogId() + ":unittest_document.cpp:TestBody : TEST", logBridge->mLog);

    ASSERT_NE(session1->getLogId(), session2->getLogId());

    LoggerFactory::instance().reset();
}

const char* NESTED_REPEATED_IMPORT = R"apl({
    "type": "APL",
    "version": "1.0",
    "import": [
      {
...
TEST(DocumentTest, NestedRepeatedImportPendingDoesNotRequest)
{
    auto doc = Content::create(NESTED_REPEATED_IMPORT, makeDefaultSession());
    ASSERT_TRUE(doc);
    ASSERT_TRUE(doc->isWaiting());
    auto requested = doc->getRequestedPackages();
    ASSERT_EQ(2, requested.size());
...
TEST(DocumentTest, NestedRepeatedImportLoadedDoesNotRequest)
{
    auto doc = Content::create(NESTED_REPEATED_IMPORT, makeDefaultSession());
    ASSERT_TRUE(doc);
    ASSERT_TRUE(doc->isWaiting());
...
TEST(DocumentTest, RepeatedImportDifferentSources)
{
    auto doc = Content::create(REPEATED_IMPORT_DIFFERENT_SOURCES, makeDefaultSession());
    ASSERT_TRUE(doc);
    ASSERT_TRUE(doc->isWaiting());
    auto requested = doc->getRequestedPackages();
    ASSERT_EQ(1, requested.size());
