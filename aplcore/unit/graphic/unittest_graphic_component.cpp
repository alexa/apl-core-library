...
TEST_F(GraphicComponentTest, FitAndScale)
{
    ...
    for (auto& ftc : sFitTestCases) {
        ...
        root = std::static_pointer_cast<CoreRootContext>(RootContext::create(Metrics().size(1024, 800), content));
...
static const char * VECTOR_GRAPHIC_CHARACTERISTICS = R"({
  "type": "APL",
  "version": "2023.2",
  "graphics": {
    "box": {
    ...
    "items": {
      ...
      "items": [
        ...
        },
        {
          "type": "VectorGraphic",
          "source": "box",
          "onFocus": { "type": "SendEvent" }
        },
        {
          "type": "VectorGraphic",
          "source": "box",
          "onBlur": { "type": "SendEvent" }
        },
        {
          "type": "VectorGraphic",
          "source": "box",
...
TEST_F(GraphicComponentTest, Characteristics)
{
    loadDocument(VECTOR_GRAPHIC_CHARACTERISTICS);

    auto vg = component->getCoreChildAt(0);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_FALSE(vg->isFocusable());
    ASSERT_FALSE(vg->isTouchable());
    ASSERT_FALSE(vg->isActionable());

    vg = component->getCoreChildAt(1);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_TRUE(vg->isFocusable());
    ASSERT_FALSE(vg->isTouchable());
    ASSERT_TRUE(vg->isActionable());

    vg = component->getCoreChildAt(2);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_TRUE(vg->isFocusable());
    ASSERT_FALSE(vg->isTouchable());
    ASSERT_TRUE(vg->isActionable());

    vg = component->getCoreChildAt(3);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_TRUE(vg->isFocusable());
    ASSERT_FALSE(vg->isTouchable());
    ASSERT_TRUE(vg->isActionable());

    vg = component->getCoreChildAt(4);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_TRUE(vg->isFocusable());
    ASSERT_FALSE(vg->isTouchable());
    ASSERT_TRUE(vg->isActionable());

    vg = component->getCoreChildAt(5);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_TRUE(vg->isFocusable());
    ASSERT_TRUE(vg->isTouchable());
    ASSERT_TRUE(vg->isActionable());

    vg = component->getCoreChildAt(6);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_TRUE(vg->isFocusable());
    ASSERT_TRUE(vg->isTouchable());
    ASSERT_TRUE(vg->isActionable());

    vg = component->getCoreChildAt(7);
    ASSERT_EQ(kComponentTypeVectorGraphic, vg->getType());
    ASSERT_TRUE(vg->isFocusable());
    ASSERT_TRUE(vg->isTouchable());
    ASSERT_TRUE(vg->isActionable());
