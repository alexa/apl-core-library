...
TEST_F(ParseTest, Simple)
{
    ...
    result = parseAndEvaluate(*context,  "${1+3}");
    ASSERT_TRUE(result.value.isNumber());
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_TRUE(IsEqual(result.value, 4));
    ASSERT_EQ(0, result.symbols.size());

    
    result = parseAndEvaluate(*context,  "${Math.min(23,4)}");
    ASSERT_TRUE(result.value.isNumber());
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_TRUE(IsEqual(result.value, 4));
    ASSERT_EQ(0, result.symbols.size());
    ...
    result = parseAndEvaluate(*context, "${@red}");
    ...
    result = parseAndEvaluate(*context, "${@red}");
    ASSERT_TRUE(result.value.is<Color>());
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_TRUE(IsEqual(result.value, Color(Color::RED)));
    ASSERT_EQ(0, result.symbols.size());

    context->putUserWriteable("b", 82);
    
    result = parseAndEvaluate(*context, "${Math.max(23,44,b)}");
    ASSERT_TRUE(result.value.isNumber());
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_TRUE(IsEqual(result.value, 82));
    ASSERT_EQ(1, result.symbols.size());

    context->userUpdateAndRecalculate("b", 100, false);
    auto applied = applyDataBinding(*context, result.expression, [](const Context&, const Object& obj) {return obj;});
    ASSERT_TRUE(applied.value.isNumber());
    ASSERT_TRUE(IsEqual(applied.value, 100));
    ASSERT_EQ(1, applied.symbols.size());
}
...
TEST_F(ParseTest, DataBindingIgnoresCLocale)
{
    ...
    auto binding = evaluate(*context, " ${12.4}");
    EXPECT_EQ(12.4, binding.asNumber());

    binding = evaluate(*context, " ${12.4 + 5}");
...
static const std::vector<std::pair<std::string, std::set<std::string>>> SYMBOL_TESTS = {
    ...
    {"${a[b].c ? (e || f) : 'foo ${g}'}",   {"a", "b", "g"}},
    {"${!a[b].c ? (e || f) : 'foo ${g}'}",  {"a", "b", "e"}},
    {"${!a[b].c ? (e && f) : 'foo ${g}'}",  {"a", "b", "e", "f"}},
...
TEST_F(ParseTest, Symbols)
{
    ...
    for (auto& m : SYMBOL_TESTS) {
        auto result = parseAndEvaluate(*context, m.first);
        ASSERT_TRUE(result.expression.isEvaluable());
        ASSERT_EQ(result.symbols.size(), m.second.size());

        for (auto& symbolName : m.second) {
            ASSERT_TRUE(std::find(result.symbols.begin(), result.symbols.end(),
                                  BoundSymbol{context, symbolName}) !=
                        result.symbols.end())
                << m.first << " symbol " << symbolName;
        }
    }
}
...
TEST_F(ParseTest, UnaryNot)
{
    ...
    context->putUserWriteable("a", 99);
    auto result = parseAndEvaluate(*context, "${!a}");
    ASSERT_TRUE(IsEqual(result.value, false));
    ASSERT_TRUE(result.expression.isEvaluable());
...
TEST_F(ParseTest, Multiply)
{
    ...
    for (auto& m : MULTIPLY_NAN_TESTS) {
        ASSERT_TRUE(evaluate(*context, m).isNaN()) << m;
    }

    context->putUserWriteable("a", 99);
    context->putUserWriteable("b", 99);
    auto result = parseAndEvaluate(*context, "${a*b}");
    ASSERT_TRUE(IsEqual(result.value, 99*99));
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_EQ(2, result.symbols.size());
    ...
    for (auto& m : MULTIPLY_EVAL) {
        ...
        ASSERT_TRUE(IsEqual(m.at(2), result.expression.eval())) << m.at(1) << "*" << m.at(2);
    }
}
...
TEST_F(ParseTest, Divide)
{
    ...
    context->putUserWriteable("b", 99);
    auto result = parseAndEvaluate(*context, "${a/b}");
    ASSERT_TRUE(IsEqual(result.value, 1.0));
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_EQ(2, result.symbols.size());
    ...
    for (auto& m : DIVIDE_NAN_EVAL) {
        ASSERT_TRUE(context->userUpdateAndRecalculate("a", m.at(0), false));
        ASSERT_TRUE(context->userUpdateAndRecalculate("b", m.at(1), false));
        ASSERT_TRUE(result.expression.eval().isNaN()) << m.at(1) << "/" << m.at(2);
    }
}
...
TEST_F(ParseTest, Remainder)
{
    ...
    context->putUserWriteable("b", 99);
    auto result = parseAndEvaluate(*context, "${a%b}");
    ASSERT_TRUE(IsEqual(result.value, 0));
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_EQ(2, result.symbols.size());
    ...
    for (auto& m : REMAINDER_EVAL) {
        ASSERT_TRUE(context->userUpdateAndRecalculate("a", m.at(0), false));
        ASSERT_TRUE(context->userUpdateAndRecalculate("b", m.at(1), false));
        ASSERT_TRUE(IsEqual(m.at(2), result.expression.eval())) << m.at(0) << "%" << m.at(1);
    }

    for (auto& m : REMAINDER_NAN_EVAL) {
        ASSERT_TRUE(context->userUpdateAndRecalculate("a", m.at(0), false));
        ASSERT_TRUE(context->userUpdateAndRecalculate("b", m.at(1), false));
        ASSERT_TRUE(result.expression.eval().isNaN()) << m.at(0) << "%" << m.at(1);
    }
}
...
TEST_F(ParseTest, Add)
{
    ...
    for (auto& m : ADD_CONCATENTATE) {
        ASSERT_TRUE(IsEqual(m.second, evaluate(*context, m.first))) << m.first;
    }

    context->putUserWriteable("a", 99);
    context->putUserWriteable("b", 99);
    auto result = parseAndEvaluate(*context, "${a+b}");
    ASSERT_TRUE(IsEqual(result.value, 198));
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_EQ(2, result.symbols.size());
...
TEST_F(ParseTest, Compare)
{
    ...
    for (auto& m : COMPARE_EVAL) {
        ...
        ASSERT_TRUE(IsEqual(target, result.expression.eval())) << m.at(0) << "<" << m.at(1);
    }

    // Greater-than
    
    context->userUpdateAndRecalculate("a", 99, false);
    context->userUpdateAndRecalculate("b", 99, false);
    result = parseAndEvaluate(*context, "${a>b}");
    ASSERT_TRUE(IsEqual(result.value, false));
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_EQ(2, result.symbols.size());
    ...
    context->userUpdateAndRecalculate("b", 99, false);
    result = parseAndEvaluate(*context, "${a<=b}");
    ASSERT_TRUE(IsEqual(result.value, true));
    ASSERT_TRUE(result.expression.isEvaluable());
    ASSERT_EQ(2, result.symbols.size());
