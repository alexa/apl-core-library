...
public:
    ...
    static Object
    eval(const char *expression, int width=1024, int height=800, int dpi=160)
    {
...
{
    auto context = Context::createTestContext(Metrics().size(1024,800), RootConfig());
...
// Each evaluation step should return the next object in the array
static const auto INLINE_DELAY_TESTS = std::vector<std::vector<Object>> {
    {"#{}", "${}", ""},
    {"#{'#{}'}", "${'#{}'}", "${}", ""},
    {"A_#{1}", "A_${1}", "A_1"},
    {"B_#{1+2}", "B_${1+2}", "B_3"},
    ...
    {"D_${'${1+3}'}", "D_4" },
    {"E_${'#{1+3}'}", "E_${1+3}", "E_4" },
    {"F_#{'#{1+3}'}", "F_${'#{1+3}'}", "F_${1+3}", "F_4" },
    {"G_#{'H_#{1+3}' + 'I_#{2+4}'}", "G_${'H_#{1+3}' + 'I_#{2+4}'}", "G_H_${1+3}I_${2+4}", "G_H_4I_6"},
    {"I_${'${1+3}:#{1+2}'}", "I_4:${1+2}", "I_4:3"},
    {"J_#{2+3} ${eval(2+3)}", "J_${2+3} 5", "J_5 5"},
    {"K_${eval('#{2+3}')}", "K_5"},
    {"L_#{eval('#{eval(2+3)}')}", "L_${eval('#{eval(2+3)}')}", "L_5"},
};

TEST_F(GrammarTest, Delayed)
{
    auto c = Context::createTestContext(Metrics(), RootConfig());

    for (const auto& m : INLINE_DELAY_TESTS) {
        for (size_t i = 0 ; i < m.size() - 1 ; i++) {
            auto result = evaluate(*c, m.at(i));
            ASSERT_TRUE(IsEqual(m.at(i + 1), result))
                << " index=" << i << " start=" << m.at(i) << " evaluated=" << result
                << " expected=" << m.at(i + 1);
        }
    }
}
