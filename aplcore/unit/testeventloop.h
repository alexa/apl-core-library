...
protected:
    ...
    void inflate() {
        ...
        root = std::static_pointer_cast<CoreRootContext>(RootContext::create(metrics, content, *config, createCallback));
...
public:
    ...
    CoreRootContextPtr root;
    DocumentContextPtr rootDocument;
    ContextPtr context;
    Metrics metrics;
...
class TestEventCommand : public TemplatedCommand<TestEventCommand> {
public:
    COMMAND_CONSTRUCTOR(TestEventCommand);
    ...
    void wrap(const char *name, int index)
    {
        ...
        CommandFactory::instance().set(name,
                                       [&, index](const ContextPtr& context,
                                           ...
                                           auto ptr = sCommandCreatorMap.at(index)(context, std::move(commandData), std::move(props), base, parentSequencer);
                                           if (!ptr)
                                               return ptr;
...
::testing::AssertionResult TransformComponent(const RootContextPtr& root, const std::string& id, Args... args) {
    auto component = root->findComponentById(id);
    if (!component)
    ...
    root->topDocument()->executeCommands(ObjectArray{event}, true);
    root->clearPending();
...
{
    if (event == Event(eventType, std::move(bag), component))
        return ::testing::AssertionSuccess();
...
template<class... Args>
::testing::AssertionResult
CheckDirtyVisualContext(const RootContextPtr& root, Args... args)
{
...
inline
::testing::AssertionResult
CheckUpdatedChildrenNotification(
    const RootContextPtr& root,
    const CoreComponentPtr& comp,
    const std::vector<ObjectMap>& change) {
    auto dirty = root->getDirty();
    if (!dirty.count(comp))
        return ::testing::AssertionFailure()
               << "No dirty property set.";

    if (!comp->getDirty().count(kPropertyNotifyChildrenChanged))
        return ::testing::AssertionFailure()
               << "No NotifyChildrenChanged property set.";

    auto notify = comp->getCalculated(kPropertyNotifyChildrenChanged);
